export default async function handler(req, res) {
  if (req.method === 'POST') {
      ...
      try {
          // Format column_values as a JSON string and escape properly
          ...
          const response = await fetch('https://api.monday.com/v2', {
              method: 'POST',
              headers: {
                  'Authorization': `Bearer eyJhbGciOiJIUzI1NiJ9.eyJ0aWQiOjQxMzc0NDI4MSwiYWFpIjoxMSwidWlkIjo2MDkzMDEyMywiaWFkIjoiMjAyNC0wOS0yMVQwODo1NToyNC4wMDBaIiwicGVyIjoibWU6d3JpdGUiLCJhY3RpZCI6ODcxMjM4MSwicmduIjoidXNlMSJ9.TS5QXrhhyI4zXSnc56XItuytJ-iklPrVB6TDF-MBdM0`,  // Use the environment variable for the monday.com API token
              ...
              body: JSON.stringify({
                  query: `
                      mutation {
                          create_item (
                              board_id: 7452641398,  // Your monday.com board ID
                              item_name: "${developerName}",  // The developer's name as the item name in the monday.com board
                              column_values: "${columnValues.replace(/"/g, '\\"')}"  // Escape double quotes for GraphQL string
                          ) {
                              id
                          }
                      }
